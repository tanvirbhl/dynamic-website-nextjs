      ...
      >
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 bg-slate-50/50">
          <div>
            ...
            <p className="text-xs text-slate-500">
              Update content and save to publish.
            </p>
          </div>
          <button
            onClick={onClose}
          ...
          >
            ...
            {section.type === "RECENT_NOTICES" && (
              <div className="p-4 bg-amber-50 border border-amber-200 rounded-lg text-amber-800 text-sm">
                ...
                <p>
                  This section automatically fetches and displays the latest
                  published notices from your database. No manual text or image
            ...
            {section.type === "LEADERSHIP_MESSAGE" && (
              <>
                <Field
                  label="Section Title (e.g., Message from the MD)"
                  name="sectionTitle"
                  register={register}
                />
            ...
            {section.type === "PARTNER_LOGOS" && (
              <>
                <Field
                  label="Section Heading (Optional, e.g., 'Trusted By')"
                  name="heading"
                  register={register}
                />
            ...
            {section.type === "IMAGE_GALLERY" && (
              <>
                <Field
                  label="Gallery Heading (e.g., 'Life at Nova')"
                  name="heading"
                  register={register}
                />
                <Field
                ...
                <h4 className="text-sm font-bold text-slate-800 border-b pb-2 mt-4">
                  Upload Photos
                </h4>
            ...
            {section.type === "CORE_VALUES" && (
              <>
                <Field
                  label="Section Heading (e.g., 'Our Mission & Values')"
                  name="heading"
                  register={register}
                />
                <Field
                ...
                <div className="space-y-6 pt-6 mt-4 border-t border-slate-100">
                  <h4 className="text-sm font-bold text-slate-800">
                    Value Blocks (Up to 4)
                  </h4>
        ...
        <div className="p-4 border-t border-slate-100 bg-white flex gap-3">
          ...
          <button
            type="button"
            onClick={onClose}
            className="flex-1 px-4 py-2 border border-slate-200 text-slate-600 rounded-md hover:bg-slate-50 transition-colors font-medium text-sm"
          >
            Cancel
          </button>
          <button
    ...
    <div>
      ...
      {type === "textarea" ? (
        <textarea
          {...register(name)}
          rows={3}
          className="w-full px-3 py-2 border border-slate-200 rounded-md text-sm outline-none focus:border-[rgb(var(--color-primary))] focus:ring-1 focus:ring-[rgb(var(--color-primary))] transition-all resize-none"
        />
      ) : (
        <input
          type="text"
          {...register(name)}
          className="w-full px-3 py-2 border border-slate-200 rounded-md text-sm outline-none focus:border-[rgb(var(--color-primary))] focus:ring-1 focus:ring-[rgb(var(--color-primary))] transition-all"
        />
      )}
    </div>
  );
